...
2. Store video and multimodal metadata in **ApertureDB** and run recognition on it. fileciteturn0file2
3. Store conversational / user / agent context in **MemMachine** to make the system “remember” past sessions. fileciteturn0file4
4. Transcribe audio with **Telnyx Speech‑to‑Text** (OpenAI‑style endpoint) to get text + timestamps. fileciteturn0file3
5. Instrument the AI parts with **Comet / Opik** for experiment tracking and evaluation. fileciteturn0file1
6. Visualize everything in a simple **Flutter** app.
7. Auto‑produce daily → weekly → monthly recap clips.

> **Hardware note:** the Freenove ESP32 kit gives us an ESP32‑WROVER and includes camera web server examples over Wi‑Fi, so the *first* step is to make that camera stream/upload. The board itself does **not** have GPS, and in this kit we treat **audio + GPS** as coming from the **phone**. Audio capture on ESP32 is possible with an I2S mic, but that’s outside first MVP scope. fileciteturn0file0
...
- Stores raw media and metadata into **ApertureDB** (images, video segments, embeddings, tags). fileciteturn0file2
- Sends audio files/URLs to **Telnyx** for transcription, stores text + timestamps. fileciteturn0file3
- Calls **MemMachine** API to persist episodic/profile memories — e.g. “user was at Cafe Venetia 2025‑11‑04 15:10, met person X”. fileciteturn0file4
- Reports traces / evals to **Comet/Opik** so we can compare recognition pipelines. fileciteturn0file1
...
3. **Node.js → ApertureDB** — accept upload, create objects, run recognition, store metadata. fileciteturn0file2
4. **Node.js → MemMachine** — for every uploaded media record a memory “user X at time T did Y at location L”. fileciteturn0file4
5. **Node.js → Telnyx** — send audio, get text, attach to same media object. fileciteturn0file3
